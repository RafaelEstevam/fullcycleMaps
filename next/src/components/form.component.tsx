...
const Form = () => {
  ...
  const handleSearchDirections = async ({
    ...
    const response = await fetch(
      `http://localhost:3000/api/directions?originId=${origin_id}&destinationId=${destination_id}`
    );
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const [sourceResponse, destinationResponse] = await Promise.all([
      fetch(`http://localhost:3000/api/places?text=${formdata.source}`),
      fetch(`http://localhost:3000/api/places?text=${formdata.destination}`),
    ]);
  ...
  return (
    <form
      ...
      className="flex justify-center bg-slate-300 p-4"
    >
      <input
        name={"source"}
        id={"source"}
      ...
      />
      <button type="submit" className="p-4 bg-cyan-500 text-white">
        Search places
      </button>
    </form>
  );
